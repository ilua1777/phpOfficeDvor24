...
abstract class AbstractContainer extends AbstractElement
{
	...
	public function lastToOne()
	{
		$tmpArray = $this->elements;
		$tmpArray2 = [];
		array_push($tmpArray2, $tmpArray[count($tmpArray) - 1]);
		for ($i = 0; $i < count($tmpArray) - 2; $i++) {
			array_push($tmpArray2, $tmpArray[$i]);
		}
		$this->elements = $tmpArray2;
	}
}
